...
impl<'a> ProxyStream<'a> {
    ...
    pub async fn process(&mut self) -> Result<()> {
        ...
        if peeked_buffer.len() < 58 {
            console_log!("buffer too short to determine protocol");
            return Err(Error::RustError("Incomplete buffer".into()));
        }
    ...
    resolve(&addr).await.map_err(|e| Error::RustError(e.to_string()))?
};
let mut remote_socket = Socket::builder().connect(addr_ip, port).map_err(|e| {
    ...
    pub async fn handle_udp_outbound(&mut self) -> Result<()> {
        // Baca query DNS dari WebSocket
        let mut buf = [0u8; 1024];
        let n = self.read(&mut buf).await?;
        let query = &buf[..n];

        // Tangani hasil DoH dengan match
        match crate::dns::doh(query).await {
            Ok(response) => {
            ...
            Err(e) => {
                console_log!("DNS resolve failed: {:?}", e);
...
impl<'a> AsyncWrite for ProxyStream<'a> {
    ...
    fn poll_shutdown(self: Pin<&mut Self>, _: &mut Context<'_>) -> Poll<tokio::io::Result<()>> {
        Poll::Ready(Ok(())) // graceful shutdown
    }
}
